...
def wait_for_setting_table():
    log("[shared_mqtt] Waiting for 'setting_table' data...")
    ...
    while True:
        ...
        if "setting_table" in shared_data:
            try:
                ...
                log("Invalid JSON for setting_table. Ignoring this message.")
...
if __name__ == "__main__":
    ...
    try:
        ...
        log(f"[SETTINGS] Big blind: {bb} Small blind: {sb} Money: {money}")
        ...
        while len([p for p in game.players if p.money > 0]) > 1:
            ...
            log(f"Remaining players: {len(game.players)}")
            time.sleep(2)

        if game.players:
            winner = game.players[0]
            log(
                f"\n [WINNER] Tournament Winner: Player on channel {winner.channel} ({winner.position}) with ${winner.money}"
            )
